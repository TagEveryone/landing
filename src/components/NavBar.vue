...
// Funzione per ottenere il path senza prefisso lingua
...
const changeLanguage = (lang: string) => {
  ...
  // Costruisci il nuovo path con la lingua
  let newPath: string;

  // Per altre lingue, aggiungi il prefisso
  newPath = currentPath === '/' ? `/${lang}` : `/${lang}${currentPath}`;

  // Naviga al nuovo path mantenendo l'hash se presente
  router.push({
    path: newPath,
    hash: currentHash
  });
...
const navLinks = [
  { key: 'home', to: '/', isRouter: true, hash: '' },
  { key: 'features', to: '/#features', isRouter: false, hash: '#features' },
  { key: 'contact', to: '/#contact', isRouter: true, hash: '#contact' }
];
</script>
    ...
    <div class="container mx-auto flex justify-between items-center">
      ...
      <div class="hidden md:flex items-center space-x-4">
        <template v-for="(link, index) in navLinks" :key="index">
          ...
          <a v-else :href="link.to" @click="(e) => handleHashClick(e, link.hash)" class="text-gray-300 hover:text-white transition">
            {{ $t(`navbar.${link.key}`) }}
          </a>
        </template>
        ...
        <div class="relative ml-2 group">
          <button @click="toggleLangDropdown"
            ...
            <span class="w-6 h-4 flex items-center justify-center"
              v-html="availableLanguages.find((lang: Language) => lang.code === locale)?.flagSvg"></span>
            <span class="hidden lg:inline">{{availableLanguages.find((lang: Language) => lang.code === locale)?.name
            }}</span>
          ...
          <div v-if="isLangDropdownOpen"
            ...
            <button v-for="lang in availableLanguages" :key="lang.code"
              ...
              :class="{ 'bg-gray-700': locale === lang.code }">
              <div class="flex items-center space-x-2">
      ...
      <div class="container mx-auto py-3 px-4">
        <div class="flex flex-col space-y-3">
          <template v-for="(link, index) in navLinks" :key="index">
            ...
            <a v-else :href="link.to" @click="(e) => { handleHashClick(e, link.hash); closeMobileMenu(); }"
              class="text-gray-300 py-2 px-3 hover:bg-gray-800 rounded-md transition">
              {{ $t(`navbar.${link.key}`) }}
            </a>
          </template>
          ...
          <div class="pt-2 pb-1 border-t border-gray-800 mt-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <button v-for="lang in availableLanguages" :key="lang.code" @click="changeLanguage(lang.code)"
                ...
                :class="locale === lang.code ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'">
